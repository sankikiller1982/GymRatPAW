            ...
            <div className="flex-1 overflow-y-auto space-y-1">
              {filteredExercises.map(ej => (
                <button key={ej.id} onClick={() => addExercise(ej)} className="w-full text-left p-3 rounded-xl hover:bg-gym-dark-border transition-colors flex items-center gap-3">
                  {ej.imagen ? (
                    <img src={ej.imagen} alt={ej.nombre} className="w-10 h-10 rounded-lg object-cover flex-shrink-0 bg-gym-dark" onError={e => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex' }} />
                  ) : null}
